...
# ───────────────────────────────────────────────────────────────
#  Utilidad: Multiselect que recuerda tu búsqueda
# ───────────────────────────────────────────────────────────────
def persistent_multiselect(label, options, key):
    """
    Igual que st.multiselect, pero vuelve a colocar el texto que habías
    escrito en la barra de búsqueda interna tras cada selección.
    """
    # 1. Componente nativo
    seleccion = st.multiselect(label, options, key=key)

    # 2. Hack: volver a inyectar el último término buscado
    #    • se guarda en session_state[f"_query_{key}"]
    #    • JS escucha el evento 'keyup' y escribe en sessionStorage
    query_key = f"_query_{key}"
    if query_key not in st.session_state:
        st.session_state[query_key] = ""

    # 2a. Capturamos lo que el usuario escriba dentro del multiselect
    st.components.v1.html(
        f"""
        <script>
        const box = window.parent.document.querySelectorAll(
            'div[data-testid="stMultiSelect"] input[type="text"]'
        )[{len(st.session_state) - 1}];  // usa el último renderizado
        if (box) {{
            // Rellena con lo que guardamos antes
            box.value = {json.dumps(st.session_state[query_key])};

            // Cuando el usuario teclea algo nuevo, lo recordamos
        ...
        // Listener que recibe el mensaje desde Streamlit
        window.addEventListener('message', e => {{
            if (e.data.type === 'restoreQuery' && e.data.key === '{query_key}') {{
                const newVal = e.data.value || '';
                if (box) box.value = newVal;
            }}
        }});
        </script>
        """,
    ...
    # 2b. En el backend: guarda y vuelve a mandar el valor
    #     (para el próximo rerender)
    if "_received" not in st.session_state:
        st.session_state._received = {}
    # Mensajes que llegan del navegador
    for msg in st.session_state._received.pop(query_key, []):
        st.session_state[query_key] = msg

    # Bridge → Streamlit ↔︎ JS
    st.components.v1.html(
        f"""
        <script>
        // Envía hacia Streamlit lo que está guardado en sessionStorage
        const last = sessionStorage.getItem('{query_key}') || '';
        window.parent.postMessage({{
            type: 'restoreQuery',
            key: '{query_key}',
            value: last
        }}, '*');
        // Guarda lo que llega desde la página principal
        window.addEventListener('message', e => {{
            if (e.data.type === 'storeQuery' && e.data.key === '{query_key}') {{
                sessionStorage.setItem('{query_key}', e.data.value);
            }}
        }});
        </script>
        """,
...
def mostrar_totales_por_categoria(results, SOLUTIONS, HOURS_FULL,
    ...
    for cat_idx, category in enumerate(CATEGORY_LABELS):
        # ——————————————————————————————————————————————
        # 1) Reunir soluciones con la GENTABLE de la categoría
        # ——————————————————————————————————————————————
        sols = [
        ...
        if not sols:
            ...
            # separador fino (en vez de divider)
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            continue

        # Normalizar schema y armar universo de centrales
        centrales_sets = []
        for s in sols:
        ...
        # 👉 Multiselect (con caption compacta)
        ...
        # Título DENTRO del gráfico (esquina sup-izquierda)
        fig.add_annotation(
        ...
        # Layout compacto (menos espacio arriba/abajo)
        fig.update_layout(
            title=None,  # importante: no usar título externo
            xaxis=dict(
            ...
            margin=dict(l=40, r=10, t=28, b=20),  # ← márgenes pequeños
            height=360  # opcional: fija altura para uniformar
        )

        st.plotly_chart(fig, use_container_width=True)
        # separador fino entre gráficos (8 px)
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
...
def mostrar_analisis_termicas(
    ...
    comentario_path  = comentarios_dir / f"comentario_termicas__{nombre_resultado}.txt"
